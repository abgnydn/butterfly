...
All code, all results, all per-generation traces — pure JS, no dependencies — in this repository ([github.com/abgnydn/butterfly](https://github.com/abgnydn/butterfly)). The early-April origin harness (`experiment.mjs`, `transgen.mjs`, `hybrid.mjs`) and the May extensions below live side by side here:
...
Pre-registrations and outcomes were filed (with git-timestamped audit trail) in the neuropulse predictions log: [PREDICTIONS.md](https://github.com/abgnydn/neuropulse/blob/main/PREDICTIONS.md), entries P-20260512-05 and P-20260515-06. They are kept there as the canonical filing; this paper references them by id.

Everything is reproducible from this repo as-is. The embedding cache is `butterfly-embed-cache.json` (gitignored) — re-generated on first run if absent.
